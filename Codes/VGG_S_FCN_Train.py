...
VGG_S_FCNmodel = VGG_S_FCN(rng,weight=W, use_last_layer_weight=False)
...
for epoch in range(n_epoch):
    ...
    for k in range(fold_batch*4):
        ...
        loss_iter, error_iter = VGG_S_FCNmodel.train(batch_input_x,batch_input_y,lr)
        ...
        if iter % 1000 ==0:
            ...
            for k in range(fold_batch):
                ...
                loss_iter, error_iter= VGG_S_FCNmodel.test(batch_input_x,batch_input_y)
            ...
            if tst_error < best_tst_error:
                ...
                file = os.listdir('VGG_S_Mean_tstfold5_1')
                if len(file)!=0:
                    os.remove('VGG_S_Mean_tstfold5_1/'+file[0])

                model_file = 'VGG_S_Mean_tstfold5_1/'+str(epoch+1)+'_'+str(iter)+'.h5'
                f = h5py.File(model_file)
                for p in VGG_S_FCNmodel.params:
