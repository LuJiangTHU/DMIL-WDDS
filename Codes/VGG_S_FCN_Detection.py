...
f = h5py.File('VGG_S_soft_tstfold5_1/8_28000.h5')
W1 = f['Conv1#W'][:]
b1 = f['Conv1#b'][:]
W2 = f['Conv2#W'][:]
b2 = f['Conv2#b'][:]
...
W_soft = [W1,b1,W2,b2,W3,b3,W4,b4,W5,b5,W6,b6,W7,b7,W8,b8]


f = h5py.File('VGG_S_Hard_tstfold5_1/8_27000.h5')
W1 = f['Conv1#W'][:]
b1 = f['Conv1#b'][:]
W2 = f['Conv2#W'][:]
b2 = f['Conv2#b'][:]
...
W_max = [W1,b1,W2,b2,W3,b3,W4,b4,W5,b5,W6,b6,W7,b7,W8,b8]


f = h5py.File('VGG_S_Mean_tstfold5_1/14_48000.h5')
W1 = f['Conv1#W'][:]
b1 = f['Conv1#b'][:]
W2 = f['Conv2#W'][:]
b2 = f['Conv2#b'][:]
...
W_avg = [W1,b1,W2,b2,W3,b3,W4,b4,W5,b5,W6,b6,W7,b7,W8,b8]
...
model_soft = VGG_S_FCN(rng,weight=W_soft, use_last_layer_weight=True,agg_func='soft')
model_max =  VGG_S_FCN(rng,weight=W_max, use_last_layer_weight=True,agg_func='max')
model_avg =  VGG_S_FCN(rng,weight=W_avg, use_last_layer_weight=True,agg_func='avg')
...
soft_error_list = open('soft_error_list_s.txt','a')
max_error_list = open('max_error_list_s.txt','a')
avg_error_list = open('avg_error_list_s.txt','a')
for index in range(len(dirs_test)):
    ...
    # box approximation by opencv findContours: soft
    ...
    plt.imsave('WDD_save_s/'+dirs_test[index]+'_soft_'+classname[prediction_soft[0]]+'.eps',img_soft)
    ...
    plt.imsave('WDD_save_s/'+dirs_test[index]+'_max_'+classname[prediction_max[0]]+'.eps',img_max)
    # ax[2].imshow(img_max)
    ...
    plt.imsave('WDD_save_s/'+dirs_test[index]+'_avg_'+classname[prediction_avg[0]]+'.eps',img_avg)
    fig.savefig('WDD_save_s/'+dirs_test[index]+'.eps',bbox_inches='tight', pad_inches=0.0, format='eps')
    # ax[3].imshow(img_avg)
    # plt.show()
